...
public class MatriculaServiceImpl implements MatriculaService {
    ...
    public void deletarPeloId(Long id) {
        ...
        if(usuarioService.getUsuarioLogado().getRoles().contains("ALUNO")) validator.validaAlunoLogado(matricula.getAluno());

        if(usuarioService.getUsuarioLogado().getRoles().contains("DOCENTE")) validator.validaDocenteLogado(matricula.getDisciplina().getDocente());

        if(matricula.getStatusSolicitacao().equals(StatusSolicitacao.EFETIVADA)) matricula.getDisciplina().acrescentaVaga();
    ...
    public void modificaNotaFinal(Long matriculaId, Double notaFinal) {
        ...
        if(notaFinal > 10 || notaFinal < 0) {
            throw new RegistroConflitanteException("Nota deve estar entre 0 e 10!");
        }
    ...
    public void modificaStatusSolicitacao(Long matriculaId, StatusSolicitacao statusSolicitacao) {
        ...
        validator.validaDocenteLogado(matricula.getDisciplina().getDocente());
        if(statusSolicitacao.equals(StatusSolicitacao.EFETIVADA)) matricula.getDisciplina().decrementaVaga(); // decrementa uma vaga e acrescenta um aluno matriculado na disciplina.
    ...
    public MatriculaResponseDTO efetivarHistorico(Long id) {
        ...
        if(!matricula.getStatusSolicitacao().equals(StatusSolicitacao.EFETIVADA)) {
            throw new RegistroConflitanteException("Proibido efetivar matrícula com status de solicitação não efetivada!");
        }
