...
class TreeViewJSXItemsPlugin {
  ...
  upsertJSXItem = (item, ownerToken) => {
    ...
    if (currentOwner != null && currentOwner !== ownerToken) {
      throw new Error(process.env.NODE_ENV !== "production" ? `MUI X: The Tree View component requires all items to have a unique \`id\` property.
Alternatively, you can use the \`getItemId\` prop to specify a custom id for each item.
Two items were provided with the same id in the \`items\` prop: "${item.id}"` : (0, _formatErrorMessage2.default)(195, item.id));
    }
  ...
  setJSXItemsOrderedChildrenIds = (parentId, orderedChildrenIds) => {
    ...
    if (parentId !== null && _selectors.selectionSelectors.isMultiSelectEnabled(this.store.state) && _selectors.selectionSelectors.propagationRules(this.store.state).descendants && _selectors.selectionSelectors.isItemSelected(this.store.state, parentId)) {
