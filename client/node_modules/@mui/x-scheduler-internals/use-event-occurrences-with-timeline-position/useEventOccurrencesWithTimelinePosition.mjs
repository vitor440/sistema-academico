 ...
 */
export function useEventOccurrencesWithTimelinePosition(parameters) {
  const {
    occurrences,
    maxSpan
  } = parameters;
  ...
  return React.useMemo(() => {
    ...
    const occurrencesWithPosition = sortedOccurrences.map(occurrence => _extends({}, occurrence, {
      position: {
 ...
 */
export function computeOccurrencesMaxIndex(adapter, occurrences) {
  const sortedOccurrences = sortEventOccurrences(occurrences);
  const conflicts = buildOccurrenceConflicts(adapter, sortedOccurrences);
 ...
 */
export function computeOccurrencesFirstIndexLookup(adapter, occurrences) {
  const sortedOccurrences = sortEventOccurrences(occurrences);
  const conflicts = buildOccurrenceConflicts(adapter, sortedOccurrences);
