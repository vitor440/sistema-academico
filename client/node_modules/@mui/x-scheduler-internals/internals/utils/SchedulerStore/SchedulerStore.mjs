...
import { buildEventsState, buildResourcesState, createEventModel, getCustomEventProperties, getUpdatedEventModelFromChanges, shouldUpdateOccurrencePlaceholder } from "./SchedulerStore.utils.mjs";
import { dateToEventString } from "../date-utils.mjs";
import { extractStandaloneEvent } from "../extractStandaloneEvent.mjs";
...
export class SchedulerStore extends Store {
  ...
  constructor(parameters, adapter, instanceName, mapper, recurringEventsPlugin = null) {
    ...
    const schedulerInitialState = _extends({}, SchedulerStore.deriveStateFromParameters(parameters, adapter), parameters.dataSource ? _extends({}, MOCK_EVENT_STATE, {
      eventModelStructure: parameters.eventModelStructure ?? {}
    }) : buildEventsState(parameters, adapter, stateFromParameters.displayTimezone, recurringEventsPlugin), buildResourcesState(parameters), {
  ...
  updateStateFromParameters = (parameters, adapter) => {
    ...
    const updateModel = (mutableNewState, controlledProp, defaultProp) => {
      ...
      if (process.env.NODE_ENV !== 'production') {
        ...
        if (initialIsControlled !== isControlled) {
          warnOnce([`MUI X Scheduler: A component is changing the ${initialIsControlled ? '' : 'un'}controlled ${controlledProp} state of ${this.instanceName} to be ${initialIsControlled ? 'un' : ''}controlled.`, 'Elements should not switch from uncontrolled to controlled (or vice versa).', `Decide between using a controlled or uncontrolled ${controlledProp} element for the lifetime of the component.`, "The nature of the state is determined during the first render. It's considered controlled if the value is not `undefined`.", 'More info: https://fb.me/react-controlled-components']);
        } else if (JSON.stringify(initialDefaultValue) !== JSON.stringify(defaultValue)) {
          warnOnce([`MUI X Scheduler: A component is changing the default ${controlledProp} state of an uncontrolled ${this.instanceName} after being initialized. `, `To suppress this warning opt to use a controlled ${this.instanceName}.`]);
        }
      }
    };
    ...
    if (!parameters.dataSource && (parameters.events !== this.parameters.events || parameters.eventModelStructure !== this.parameters.eventModelStructure || adapter !== this.state.adapter)) {
      Object.assign(newSchedulerState, buildEventsState(parameters, adapter, newSchedulerState.displayTimezone, this.state.recurringEventsPlugin));
    }
  ...
  updateEvents(parameters) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      for (const id of deleted) {
        if (updated.has(id)) {
          warnOnce([`MUI X Scheduler: id "${String(id)}" appears in both \`deleted\` and \`updated\`.`, 'These two arrays must be disjoint, otherwise the order of operations is undefined.']);
        }
      }
    }
  ...
  createEvent = calendarEvent => {
    if (this.state.recurringEventsPlugin == null && calendarEvent.rrule) {
      ...
      return this.updateEvents({
        created: [_extends({}, calendarEvent, {
          rrule: undefined
        })]
      }).created[0];
    }
  ...
  updateEvent = calendarEvent => {
    ...
    if (this.state.recurringEventsPlugin != null && original.dataTimezone.rrule) {
      throw new Error(process.env.NODE_ENV !== "production" ? 'MUI X Scheduler: This event is recurring and cannot be updated with updateEvent(). ' + 'Recurring events require special handling to manage series and exceptions. ' + 'Use updateRecurringEvent() instead to update recurring events.' : _formatErrorMessage(222));
    }
    if (this.state.recurringEventsPlugin == null && calendarEvent.rrule != null) {
      ...
      this.updateEvents({
        updated: [_extends({}, calendarEvent, {
          rrule: undefined
        })]
      });
      return;
  ...
  updateRecurringEvent = params => {
    ...
    this.set('pendingRecurringEventOperation', _extends({
      kind: 'update'
    }, params));
  };
  ...
  selectRecurringEventScope = scope => {
    ...
    if (!original.dataTimezone.rrule) {
      throw new Error(process.env.NODE_ENV !== "production" ? 'MUI X Scheduler: The event targeted by the recurring scope dialog is not recurring. ' + 'Recurring scope changes require an event with a recurrence rule. ' + 'Use updateEvent() or deleteEvent() for non-recurring events.' : _formatErrorMessage(223));
    }
  ...
  pasteEvent = changes => {
    ...
    const _original$modelInBuil = original.modelInBuiltInFormat,
      ...
      copiedEventWithoutId = _objectWithoutPropertiesLoose(_original$modelInBuil, _excluded);
    const dataTimezone = original.modelInBuiltInFormat.timezone ?? 'default';
    const stringifiedChanges = _extends({}, cleanChanges);
    if (cleanChanges.start != null) {
      stringifiedChanges.start = dateToEventString(adapter, cleanChanges.start, original.modelInBuiltInFormat.start, dataTimezone);
    }
    if (stringifiedChanges.end != null) {
      stringifiedChanges.end = dateToEventString(adapter, stringifiedChanges.end, original.modelInBuiltInFormat.end, dataTimezone);
    }
    const createdEvent = _extends({}, copiedEventWithoutId, stringifiedChanges, {
      extractedFromId: id
    });
    return this.updateEvents({
