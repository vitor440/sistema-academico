...
export function TimeGridColumn(props) {
  ...
  const start = React.useMemo(() => adapter.setHours(adapter.startOfDay(day.value), startTime), [adapter, day, startTime]);
  const end = React.useMemo(() => endTime === 24 ? adapter.endOfDay(day.value) : adapter.setHours(adapter.startOfDay(day.value), endTime), [adapter, day, endTime]);
